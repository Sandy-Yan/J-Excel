...
public class SimpleExportService extends BaseExcelService {
    ...
    private boolean createHead = true;

    public SimpleExportService(Sheet sheet, String[] fieldNames, String title) {
     ...
     * 执行导出
     *
     * @param data 数据可以是List<VO>也可以是List<Map>
     * @return
     */
    public SimpleExportService doExport(List data) {
        createHead();
        if (ObjectHelper.isNotEmpty(data)) {
            for (int r = 0; r < data.size(); r++) {
                Object object = data.get(r);
                log.debug("写入第{}行", CURRENT_SPACE_ROW);
                Row row = createRow(sheet, CURRENT_SPACE_ROW, fieldNames.length);
